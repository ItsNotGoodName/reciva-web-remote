...
func HandleWS(bus dto.Bus) func(conn *websocket.Conn) {
...
func wsHandleWrite(ctx context.Context, cancel context.CancelFunc, conn *websocket.Conn) chan<- dto.Command {
	writeC := make(chan dto.Command)
	go func() {
...
func wsHandleRead(ctx context.Context, cancel context.CancelFunc, conn *websocket.Conn) <-chan dto.Command {
	readC := make(chan dto.Command)
	go func() {
		defer cancel()
